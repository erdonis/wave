  ...
  const submitEmail = useCallback(async () => {
    ...
    if (validateEmail.success) {
      ...
      if (mode === 'register') {
        ...
        continueLogin = res.ok;
        if (!res.ok) {
          showToast({ title: 'Error', message: (await res.json()).msg, type: 'error' });
        }
      }
      if (continueLogin) {
      ...
      showToast({ title: 'Error', message: validateEmail.error.issues[0].message, type: 'error' });
      setLoading((prev) => ({
        ...prev,
        email: false,
      }));
    }
  }, [email, mode, callbackUrl, showToast]);
